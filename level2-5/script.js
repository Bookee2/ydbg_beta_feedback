...
const BETA_TESTER_ROSTER = {
    ...
    "Level 4": [
        // TODO: Add Level 4 tester names here
    ],
    "Level 5": [
        // TODO: Add Level 5 tester names here
    ]
};
...
// Beta Group change handler - filter names by selected group
betaGroupSelect.addEventListener('change', (e) => {
    ...
    if (!selectedGroup) {
        // No group selected - disable name dropdown
        nameSelect.disabled = true;
        nameSelect.innerHTML = '<option value="">Select group first…</option>';
    } else {
        // Group selected - enable and populate name dropdown
        nameSelect.disabled = false;
        nameSelect.innerHTML = '<option value="">Select name…</option>';
        
        const names = BETA_TESTER_ROSTER[selectedGroup] || [];
        names.forEach(name => {
        ...
        if (names.length === 0) {
            console.warn(`No names found for ${selectedGroup}. Please update BETA_TESTER_ROSTER.`);
        }
    }
});
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const feedbackData = {
            betaGroup: formData.get('betaGroup'),
        ...
        console.log('Final feedback data:', {
            betaGroup: feedbackData.betaGroup,
...
function validateForm() {
    ...
    console.log('Validating form:', { betaGroup, name, os, feedbackType, detailsLength: details.length });
    
    if (!betaGroup || !name || !os || !feedbackType || !details) {
...
async function sendToSlack(data) {
    ...
    try {
        ...
        const slackMessage = {
            text: `New YDBG App Beta Feedback from ${data.name} (${data.betaGroup})`,
            blocks: [
                {
                    type: "header",
                    text: {
        ...
        const approaches = [
            ...
            // Approach 2: Simple message format
            {
                ...
                options: {
                    ...
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        text: `🔍 New Beta Feedback from ${data.name} (${data.betaGroup}, ${data.os}): ${data.details}`
                    })
                }
            },
...
function submitAnother() {
    ...
    // Reset name dropdown to disabled state
    nameSelect.disabled = true;
    nameSelect.innerHTML = '<option value="">Select group first…</option>';
